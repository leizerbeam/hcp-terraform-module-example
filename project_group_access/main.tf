...
data hcp_group "api_token_group" {
  resource_name = var.api_token_group_name 
}
...
#  principal_id   = var.api_token_group_name # hardcoded name of team api token
...
data tfe_team "provider_test_tfe_team" {
  ...
  organization   = var.organization_name # could be possibly substituted with data.hcp_organization.name
}
